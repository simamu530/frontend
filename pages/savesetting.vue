...
export default {
  ...
  methods: {
    submit() { //保存ボタンを押したら呼び出される
      ...
      this.$axios.$post('http://127.0.0.1:8000/api/v1/savesetting', sendData)
      .then(this.asyncData)
    ...
    deleteItemConfirm () {
      const testid = Object.assign(this.items[this.editedIndex].id);
        ...
        try {
          this.$axios.delete('http://127.0.0.1:8000/api/v1/savesetting/' + testid, sendData)
          .then( res => {
            console.log(res);
            console.log("success");
          }
    ...
    async asyncData(){
      this.loading = true;
      await this.$axios.$get('http://127.0.0.1:8000/api/v1/savesetting')
    .then(res => {
      this.items = res.data;
      console.log(res)
      this.loading = false;
